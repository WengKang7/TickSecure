...
window.addEventListener('ts-auth-ready', async () => {
    ...
    const renderEvents = (events) => {
        ...
        grid.innerHTML = events.map(e => {
            const date = e.startDate ? new Date(e.startDate).toLocaleDateString() : '';
            const status = e.status === 'published' ? 'Available' : 'Limited';
            const tone = status === 'Limited' ? 'warning' : 'success';
            const priceStr = e.lowestPrice ? `From RM${e.lowestPrice}` : 'TBA';
            return `
