...
function render_public_page(string $title, string $active, string $content, string $base='.', bool $buyerLogged=true): void {
  $eventActive=$active==='events'?'active':''; $resaleActive=$active==='resale'?'active':''; $ticketsActive=$active==='tickets'?'active':'';
  ...
  echo '<button class="ts-icon-btn ts-mobile-menu-btn">'.ts_icon('menu').'</button></div></div></header>'.$content.ts_modal_and_toast().'<script src="'.$base.'/assets/js/ui.js"></script></body></html>';
}
...
function ts_sidebar_links(string $role, string $active, string $base): string {
  ...
  $html='';
  foreach($groups as $label=>$items){$html.='<div class="ts-nav-group"><div class="ts-nav-label">'.htmlspecialchars($label).'</div>';foreach($items as $it){[$key,$label2,$icon,$file]=$it;$html.='<a class="ts-side-link '.($active===$key?'active':'').'" href="'.$base.'/'.$role.'/'.$file.'">'.ts_icon($icon).'<span>'.$label2.'</span></a>';}$html.='</div>';}
  return $html;
}
...
function render_dashboard_page(string $role, string $title, string $active, string $content, string $base='..', string $breadcrumb=''): void {
  ...
  echo '<aside class="ts-sidebar"><div class="ts-sidebar-head">'.ts_brand($base).'</div><nav class="ts-sidebar-nav">'.ts_sidebar_links($role,$active,$base).'</nav><div class="ts-sidebar-foot"><div class="ts-profile-card"><div class="ts-avatar">'.$initial.'</div><div class="ts-profile-meta"><div class="ts-profile-name">'.$person.'</div><div class="ts-profile-role">'.$roleName.'</div></div></div></div></aside>';
  echo '<main class="ts-main"><header class="ts-dashboard-header"><div class="flex items-center gap-12"><button class="ts-icon-btn ts-sidebar-toggle">'.ts_icon('menu').'</button><div><div class="ts-breadcrumb">'.htmlspecialchars($breadcrumb ?: $roleName).'</div><div class="ts-dashboard-title">'.htmlspecialchars($title).'</div></div></div><div class="ts-dashboard-actions"><button class="ts-icon-btn">'.ts_icon('search').'</button><button class="ts-icon-btn">'.ts_icon('bell').'</button><span class="ts-role-pill">'.$roleName.'</span><div class="ts-avatar">'.$initial.'</div></div></header><div class="ts-page">'.$content.'</div></main></div>'.ts_modal_and_toast().'<script src="'.$base.'/assets/js/ui.js"></script></body></html>';
}
