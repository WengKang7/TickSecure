...
<div class="text-center">
    <div class="ts-success-icon" style="background:var(--brand-100);color:var(--brand-900)"><?=ts_icon('mail','ts-icon-xl')?></div>
    <h1 class="ts-auth-title">Verify your email</h1>
    <p class="ts-auth-sub">We sent a verification link to <strong id="verification-email">your email address</strong>. Open it, then return here to continue.</p>
    ...
    <button class="ts-btn ts-btn-secondary w-full mt-8" id="verification-resend" type="button">Resend verification email</button>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const dashboardFor = (profile) => {
        if (profile?.role === 'admin') return '../admin/dashboard.php';
        if (profile?.role === 'organizer') return '../organizer/dashboard.php';
        return '../public/events.php';
    };
    ...
    const syncVerifiedEmail = async () => {
        ...
        if (!verified) return { verified: false, profile: null };
        const profile = await window.tsDb.getUserProfile(user.uid);
        if (!profile) throw new Error('Your user profile could not be found. Please contact support.');
    ...
    completeButton?.addEventListener('click', async () => {
        ...
        try {
            const { verified, profile } = await syncVerifiedEmail();
            if (!verified) {
                message.textContent = 'Your email is not verified yet. Open the latest verification link, then try again.';
                return;
            }

            if (profile.status !== 'active') {
                message.textContent = profile.role === 'organizer'
                    ? 'Your email is verified. Your organizer application is still awaiting administrator approval.'
                    : 'Your email is verified, but your account is not active. Please contact support.';
                return;
            }
            window.location.replace(dashboardFor(profile));
        } catch (error) {
            const detail = error?.message || 'Unable to sync verification status.';
            message.textContent = detail === 'internal'
                ? 'Your email is verified, but the account record could not be synchronized. Please try again or check the local backend setup.'
                : detail;
        } finally {
    ...
    returnButton?.addEventListener('click', async (event) => {
        ...
        try {
            ...
            try { await window.tsAuth.logout(); } catch (_) { /* no-op */ }
        }
